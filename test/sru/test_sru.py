...
@pytest.mark.parametrize(
    'cuda', [False, pytest.param(True, marks=pytest.mark.skipif(not torch.cuda.is_available(), reason='no cuda available'))])
@pytest.mark.parametrize(
    'with_grad', [False, True]
)
@pytest.mark.parametrize(
    'compat', [False, True]
)
def test_cell(cuda, with_grad, compat):
    ...
    def run():
        ...
        words_embeddings = torch.rand((max_len, num_sentences, embedding_size), dtype=torch.float32)
        if cuda:
        ...
        def cell_to_emb(cell, batch_size):
            if compat:
                # should arrive as: (num_layers * num_directions, batch, hidden_size)
                cell = cell.view(layers, 2 if bidirectional else 1, batch_size, rnn_hidden)
                cell = cell[-1].transpose(0, 1)
                # (batch, num_directions, hidden_size)
                cell = cell.contiguous().view(batch_size, -1)
            else:
                # should arrive as: (num_layers, batch_size, num_directions * hidden_size)
