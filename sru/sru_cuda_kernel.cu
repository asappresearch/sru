...
template <typename scalar_t>
__global__ void tsru_cuda_forward_kernel(
                        scalar_t* __restrict__ h,
                        const scalar_t* __restrict__ u,
                        const scalar_t* __restrict__ weight_c,
                        const scalar_t* __restrict__ bias,
                        ...
                        const int d)
{
    ...
    const int ncols_u = ncols*2;

    const auto wc1 = *(weight_c + (col%d));
    const auto bias1 = *(bias + (col%d));
    auto cur = *(init + col);
    const auto* up = u + (col*2);
    const unsigned char* pad_p = (mask_pad == NULL) ? NULL : (mask_pad + (col/d));
    auto* hp = h + col;
...
template <typename scalar_t>
__global__ void tsru_cuda_backward_kernel(
                        scalar_t* __restrict__ grad_u,
                        scalar_t* __restrict__ grad_wc,
                        scalar_t* __restrict__ grad_bias,
                        scalar_t* __restrict__ grad_init,
                        ...
                        const int d)
{
    ...
    const auto* up = u + (col*2) + (len-1)*ncols_u;
    const auto* hp = h + col + (len-1)*ncols;
    const auto* ghp = grad_h + col + (len-1)*ncols;
    const unsigned char* pad_p = (mask_pad == NULL) ? NULL : (mask_pad + (col/d) + (len-1)*batch);
    auto* gup = grad_u + (col*2) + (len-1)*ncols_u;

    for (int row = len-1; row >= 0; --row)
    {
        if ((pad_p == NULL) || !(*pad_p)) {
            const auto prev_h_val = (row>0) ? (*(hp-ncols)) : (*(init+col));
            const auto u0 = *up;
            const auto u1 = *(up + 1);
            ...
            // gradient with respect to values in the first gate g1
            const auto gg1 = gh*(prev_h_val-u0)*(g1*(1.f-g1));
            gbias1 += gg1;
            gwc1 += gg1*prev_h_val;

            // gradient with respect to h[t-1]
            cur = gh*g1 + gg1*wc1;
...
void tsru_cuda_forward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(U.type(), "tsru_forward_cuda", ([&] {
        tsru_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
            h.data<scalar_t>(),
            U.data<scalar_t>(),
            weight_c.data<scalar_t>(),
            bias.data<scalar_t>(),
            h_init.data<scalar_t>(),
            mask_pad.numel() ? mask_pad.data<unsigned char>() : NULL,
            length,
            batch_size,
            hidden_size);
...
void tsru_cuda_bi_forward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(U.type(), "tsru_bi_forward_cuda", ([&] {
        tsru_cuda_bi_forward_kernel<scalar_t><<<blocks, threads>>>(
            h.data<scalar_t>(),
            U.data<scalar_t>(),
            weight_c.data<scalar_t>(),
            bias.data<scalar_t>(),
...
void tsru_cuda_backward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(U.type(), "tsru_backward_cuda", ([&] {
        tsru_cuda_backward_kernel<scalar_t><<<blocks, threads>>>(
            grad_u.data<scalar_t>(),
            grad_wc.data<scalar_t>(),
            grad_bias.data<scalar_t>(),
            grad_init.data<scalar_t>(),
...
void tsru_cuda_bi_backward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(U.type(), "tsru_bi_backward_cuda", ([&] {
        tsru_cuda_bi_backward_kernel<scalar_t><<<blocks, threads>>>(
            grad_u.data<scalar_t>(),
            grad_wc.data<scalar_t>(),
            grad_bias.data<scalar_t>(),
            grad_init.data<scalar_t>(),
