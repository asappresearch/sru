...
class SRU_Compute_GPU(Function):
    ...
    def __init__(self, activation_type, d_out, bidirectional=False, scale_x=1.0):
...
def SRU_Compute_CPU(activation_type, d, bidirectional=False, scale_x=1.0):
...
class SRUCell(nn.Module):
    ...
    def init_weight(self, rescale=True):
        ...
        if self.index>0:
            magic_number = 1.0/min(1, 0.03*self.index + 0.47)
            w[:,:,:,0].mul_(magic_number**0.5)
        if self.dropout>0:
...
class SRU(nn.Module):
    ...
    def forward(self, input, c0=None, return_hidden=True):
        ...
        for i, rnn in enumerate(self.rnn_lst):
            h, c = rnn(prevx, c0[i])
            h = h * (2**0.5)
            #rnn.var_x = prevx.data[-1].var()
            #rnn.var_h = h.data[-1].var()
            #rnn.var_c = c.data.var()
            #rnn.tmp = h.data[-1].clone()
            #prevx = h
